...
test $# -ne "3" && echo "Usage: $0 [Gene Tree File] [Species Tree File] [Output Name] [Out directory] [ASTRAL jar file]" && exit 1
...
python3 ./unrooted_species_delimitation.py $outdir/${out}2.tre $outdir/$out.cl 0.05 > /dev/null
